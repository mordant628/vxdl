...
/**
 * This class is a direct copy, with modifications, from:
 *
 *    https://raw.githubusercontent.com/vert-x3/vertx-web/master/vertx-web/src/main/java/io/vertx/ext/web/handler/sockjs/impl/EventBusBridgeImpl.java
 *
 * The deadlock is a classic A|B lock inversion:
 * <ul><li>Thread#1 first locks "A", then locks "B".</li><li>Meanwhile, Thread#2 first locks "B", then locks "A"</li></ul>
 *
 *<pre>
 *
 ...
 *
 *
 * To induce this deadlock, we need to (1) acquire the first lock on one thread, (2) let the other thread run to the 'waiting to lock' state, (3), let the first thread then continue on.
 *
 * There is no way to cleanly do this, however we can hack it.
 *
 * The SockJSSession.write() call accepts a Buffer.java object, calls the 'toString()' method on that buffer to get the string data, and then moves on. The SockJSSession.write() call is a synchronized method (the first lock on thread one, step (1)), so we want to cause that .write() call to pause so that a separate thread can then invoke the close logic.  
 *
 * Fortunately the Buffer.java is an interface, so I can create my own implementation which induces a sleep:  SleepyBuffer.java
 *
 * So the only difference between the original EventBusBridgeImpl.java impl and this class is the SleepyBuffer is used in the 'deliverMessage()' method.  Thereby, the write() call will be locked, but paused, and we can then kill the socket connection to induce the deadlock.
 * 
 */
public class DeadlockEventBusBridgeImpl implements Handler<SockJSSocket> {
  ...
  public DeadlockEventBusBridgeImpl(Vertx vertx, BridgeOptions options, Handler<BridgeEvent> bridgeEventHandler) {
    this.vertx = vertx;
  ...
  private void deliverMessage(SockJSSocket sock, String address, Message message) {
    ...
    checkCallHook(() -> new BridgeEventImpl(BridgeEventType.RECEIVE, envelope, sock),
      () -> sock.write(new SleepyBuffer(envelope.encode())),
